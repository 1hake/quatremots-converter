...
(async () => {
    ...
    const promises = resolutions.map(({ name, bitrate, maxrate, bufsize, width, height }) => {
        return new Promise((resolve, reject) => {
            const ffmpeg = spawn("ffmpeg", [
                "-i",
                "https://4mots-live.s3.fr-par.scw.cloud/IMG_1184.MOV",
                "-filter_complex",
                ...
                "h264",
                "-profile:v",
                "high", // Keep high profile for 8-bit encoding
                "-pix_fmt",
                "yuv420p", // Force 8-bit color depth
                "-crf",
                "20",
                "-sc_threshold",
                "0",
                ...
                path.join(folderPath, `${name}_%03d.ts`),
                "-f",
                "hls",
                path.join(folderPath, `${name}.m3u8`),
            ]);
            // Capture ffmpeg stderr to log errors
    ...
    try {
        console.time("execition");

        await Promise.all(promises);
        console.timeEnd("execition");
    } catch (err) {
        console.log(err);
    }
})();
